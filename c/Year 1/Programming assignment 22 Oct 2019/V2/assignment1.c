...
{
  srand(time(NULL));
  // Initialization of randomisation code
  int max = 10;
  int max_op = 4;
  ...
  char qo;
  exit=quizcount=0;
  //locks option 3 on first launch.
  while (exit==0)
  //begins the loop
  {
    ...
    //Cycles through selected options
        ...
        do {
          //gets input and only loops if the input is out of the specified range.
           ...
           //the check to make sure its within spec
				break;
      ...
      case 2:
        ...
        quizcount=1;
        //This unlocks option 3, case 3 below.
        for (i=1; i<=qnum; i++)
        {
          ans=Uans=0;
          qn1 = rand()%max;
          qn2 = rand()%max;
          //randomising both numbers in the questions

          op_select = rand()%max_op;
           //using rand num to prompt a switch
           // this switch chooses a random operator
      ...
      case 3:
  			if (quizcount==1)
        //checks if the flag has been triggered back in case 2
  			{
  				printf("Correct=%d\n\nIncorrect=%d\n\n", correct,incorrect);
  			}
  			else
